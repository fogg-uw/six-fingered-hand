...
D0a_ex should always be 0. That's if 3_2 blobs were defined as
blobs containing a 3_2 cycle, but a different definition was implemented here.
```{r}
dat[which(dat$D0a_ex > 0), c(1,4:10,13:21,37:42)]
```

3 problems: jobs 117 (2), 141 (1), 144 (2).
...
and it's not numerical innacuracy: `s1_exact`=0.3090289, the other 2 splits
...
```{r}
ggplot(filter(dat, q12>0),
       # from simulated CFs: formerly filtered by A1>0 and used y=C1_A1
       aes(y=(D0a_ex+D1a_ex)/(q12), x=factor(d_0), color=factor(mu_lambda), shape=ntaxa_M)) +
  ...
  scale_y_continuous(name=expression(proportion~anomalous~among~3[2]~blob~nets),
                     # "proportion anomalous, among 3_2-blob networks",
                     trans="sqrt",
...
ggplot(filter(dat, q10+q11+q12 > 0),
       aes(y=(D0a_ex+D1a_ex)/(q10+q11+q12), x=factor(d_0), color=factor(mu_lambda), shape=ntaxa_M)) +
  ...
  theme(plot.subtitle = element_text(hjust=0.5), legend.position = "bottom") +
  labs(x="(distance threshold d) / \u03BB") +
  scale_y_continuous(name="proportion anomalous among class-1 networks", trans="sqrt",
                     breaks=c(0,.001,.005,.01,.015,.02), minor_breaks=c(0),
                     expand=c(.002,.002))
ggsave("fig_prob_anomalous1_ex.pdf", height=4, width=7, device=cairo_pdf)
```

sample size behind each point:
```{r}
filter(dat, q10+q11+q12>0) %>%
  group_by(lambda, d_0, mu_lambda, ntaxa, M) %>%
  summarize(samplesize = sum(q10+q11+q12), .groups="drop") %>%
  group_by(ntaxa) %>%
  summarize(min_samplesize = min(samplesize), median_samplesize = median(samplesize), .groups="drop")
```

    ntaxa min_samplesize median_samplesize
    4                636              728 
    6               9111            11058 
    8              41410            51680.
...
```{r}
ggplot(filter(dat, q2>0),
       # from simulated CFs: formerly filtered by A2>0 and used y=C2_A2
       aes(y=(D2a_ex)/q2, x=factor(d_0), color=factor(mu_lambda), shape=ntaxa_M)) +
  ...
  theme(plot.subtitle = element_text(hjust=0.5), legend.position = "bottom") +
  labs(x="(distance threshold d) / \u03BB") +
  scale_y_continuous(name="proportion anomalous among class-2 networks", trans="sqrt",
...
plot_overall_anomalies = function(dat_y){ # uses 'y' as response variable name
  ...
  theme_bw() +
  theme(plot.subtitle = element_text(hjust=0.5), legend.position = "bottom") +
  labs(x="(distance threshold d) / \u03BB") +# expression(distance~threshold~d/lambda)
  scale_y_continuous(name="proportion of anomalous networks", trans="sqrt",
...
ggsave("fig_prob_anomalous12_naive_300g.pdf", height=4, width=7, device=cairo_pdf)
```
...
ggsave("fig_prob_anomalous12_lib_300g.pdf", height=4.5, width=7, device=cairo_pdf)
```
...
ggsave("fig_prob_anomalous12_cons_300g.pdf", height=4.5, width=7, device=cairo_pdf)
```

from exact qCF calculations:

```{r}
plot_overall_anomalies(mutate(dat, y = (D1a_ex+D2a_ex)/q))
ggsave("fig_prob_anomalous12_ex.pdf", height=4.5, width=7, device=cairo_pdf)
```
...
blobs containing a 3_2 cycle, but a different definition was implemented here.
```{r}
dat[which(dat$D0a_ex > 0), c(1,4:10,13:21,37:42)] # length 0
```
...
```{r}
filter(dat, lambda==3, mu==2.7, rho %in% c(0.6,1), d_0==1.2, M==0.5)[,c(1,4:10,13:14)]
```
...
```{r}
stopifnot(
  dat %>% filter(rho==1) %>% mutate(anomsum = rowSums(across(ends_with("a_ex")))) %>%
          summarize(num_anom = unique(anomsum)) == 0)
```

for averaged summary stat, use:
```{r}
qtot = 800 * (njobs/4/3) # 19200
```
...
plot_overall_anomalies_rho = function(dat_y){ # uses 'y' as response variable name
  ...
  theme_bw() +
  theme(plot.subtitle = element_text(hjust=0.5), legend.position = "bottom") +
  labs(x="inheritance correlation") +
  scale_y_continuous(name="proportion of anomalous networks", trans="sqrt",
...
ggsave("fig_prob_anomalous12_rho_naive_300g.pdf", height=4, width=7, device=cairo_pdf)
```

the liberal way:
```{r}
plot_overall_anomalies_rho(mutate(dat, y = (D1anom+D2anom)/q))
ggsave("fig_prob_anomalous12_rho_lib_300g.pdf", height=4, width=7, device=cairo_pdf)
```

the conservative way:
```{r}
plot_overall_anomalies_rho(mutate(dat, y = (D1anom+D1ambi + D2anom+D2ambi)/q))
ggsave("fig_prob_anomalous12_rho_cons_300g.pdf", height=4, width=7, device=cairo_pdf)
```

from exact qCF calculations:

```{r}
plot_overall_anomalies_rho(mutate(dat, y = (D1a_ex+D2a_ex)/q))
