...
ggplot(dat, aes(y=A1_q, x=factor(mu_lambda), color=factor(lambda), shape=ntaxa_M)) +
  ...
  scale_y_continuous(name=expression(proportion~of~networks~with~a~3[2]~blob),
...
ggplot(dat, aes(y=A2_q, x=factor(mu_lambda), color=factor(lambda), shape=ntaxa_M)) +
  ...
  theme(legend.position="bottom", legend.margin=margin(0,10,0,10)) +
  labs(x=expression(turnover~rate~mu/lambda)) +
  scale_y_continuous(name="proportion of class-2 networks") # sqrt too strong
ggsave("fig_prob_class2.pdf", height=4, width=7, device=cairo_pdf)
```

todo: merge these 2 plots into a single plot, but with a single legend.
using `grid.arrange`, `get_legend(plot)` and `+ theme(legend.position="none")`.
