...
p11 = ggplot(dat,
  ...
  scale_y_continuous(name=expression("%"~networks~with~a~3[2]~blob)) +
...
p12 = ggplot(dat,
  ...
  theme(legend.position="bottom", legend.margin=margin(0,10,0,10)) +
  labs(x=expression(turnover~rate~mu/lambda)) +
  scale_y_continuous(name="% class-2 networks")
  # trans="sqrt" is too strong
...
p21 = ggplot(filter(dat, q12>0),
  ...
  scale_y_continuous(name="% anomalous among 3-2 blobs",
                     trans="mysqrt", # "sqrt",
                     breaks=c(0,.001,.01,.05,.1,.2,.5,.75,1)*100,
...
p22 = ggplot(filter(dat, q2>0),
  ...
  theme(legend.position = "bottom") +
  labs(x="(distance threshold d) / \u03BB") +
  scale_y_continuous(name="% anomalous among class-2 networks",
                     trans="mysqrt", # "sqrt",
                     breaks=c(0,.001,.01,.05,.1,.2,.3,.4,.75,1)*100,
...
p32 = plot_overall_anomalies(mutate(dat, y = (C1+C2)/q)) +
  ylab("% observed anomalous in 300 loci")
...
plot_class_anomalies_rho =
  ...
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("inheritance correlation") + ylab(ylab) +
  scale_y_continuous(trans="mysqrt", # "sqrt",
                     breaks=c(0,.005,.01,.02,.05,.1)*100,
                     minor_breaks=c(0),
                     expand=expand)
}
```
...
p51 = plot_class_anomalies_rho(
  ...
  ylab="% anomalous among 3-2 blobs",
...
p52 = plot_class_anomalies_rho(
  ...
  ylab="% anomalous among class-2 networks",
  seed=7, dw=0.5)
